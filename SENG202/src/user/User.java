...
/**
 * This class provides an abstract version of a user profile. The profile consists of the users date of birth, name, weight, height, gender. The BMI is estimated by a commonly used
 * formula found on wikipedia. The user profile also has the event container that holds all of the activity events associated with the user.
 * 
...
public class User implements Serializable {
	...
	public User(String name, Calendar dateOfBirth, Gender gender,
		...
		userID = Persistent.generateUserID();
	}
	 ...
	 *            The resting heart rate that will be associated with the user.
	 */
	...
	/**
	 * Calculates to see if the user has any health warnings that need to be displayed. Health warning consist of Bradycaria and Tachycardia. Bradycardia is the resting heart rate
	 * of under 60 beats per minute (BPM). Tachycardia is when the heart rate exceeds a given range when exercising.
	 */
	...
	/**
	 * Calculates the BMI of the user based on their height and weight and sets it to their user profile.
	 */
	...
	public int getAge() {
		...
		if (birth.get(Calendar.MONTH) > now.get(Calendar.MONTH)
				|| (now.get(Calendar.MONTH) == birth.get(Calendar.MONTH) && birth
	...
	/**
	 * Adds events from the new event container to the users event container. The EventContainer method addEvent() will get rid of any duplicate events.
	 * 
	 * @param events
	 *            the events to add.
	 */
	public void addEvents(EventContainer events) {
		...
		LoadSummary.addEventsAdded(this.events.getAllEvents().size()
				- sizeBefore);
		// total events in event container to be added - new events added
		LoadSummary.addEventsNotAdded(FileLoader.numberOfEvents
				- (this.events.getAllEvents().size() - sizeBefore));
		Saver.SaveUser(this);
	}
	 ...
	 * 
	 * @param other
	 *            the user profile to compare for equality
	 */
	@Override
